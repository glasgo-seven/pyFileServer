...
print(f'Server [{host}] is listening on [{portFrom}] port ...')
conn, addr = sFrom.accept()		# Establish connection with client. Work once.

print(f'Connected to {addr} !')
with open('receivedFromClient.json', 'wb') as f:
	print('\tFile opened.')
	while True:
...
sTo.listen(1)						# Now wait for client connection.
print(f'Server [{host}] is listening on [{portTo}] port ...')
conn, addr = sTo.accept()		# Establish connection with client. Work once.

print(f'Got connection from {addr}!')
filename = 'receivedFromClient.json'
f = open(filename,'rb')
l = f.read(1024)
while (l):
	conn.send(l)
